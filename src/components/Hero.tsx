...
const nameChars = "Nikhil Tiwari".split("");
...
const charVariants = {
  ...
  visible: (i: number) => ({
    opacity: 1,
    y: 0,
    transition: { duration: 0.4, ease: [0.33, 1, 0.68, 1], delay: 0.2 + i * 0.05 },
  }),
};
...
const socialVariants = {
  ...
  visible: (i: number) => ({
    opacity: 1,
    scale: 1,
    transition: { duration: 0.4, ease: [0.34, 1.56, 0.64, 1], delay: 1.8 + i * 0.1 },
  }),
};
  ...
  return (
    <section id="home" className="relative min-h-screen flex items-center overflow-hidden">
      ...
      <div className="relative z-10 w-full px-6 md:px-12 lg:px-24 max-w-[1400px] mx-auto">
        ...
        <h1 className="font-display text-[clamp(2.5rem,9vw,9rem)] font-bold leading-[0.82] tracking-[-0.04em] text-[var(--text-primary)]">
          <motion.span className="inline-block" initial="hidden" animate="visible">
            {nameChars.map((char, i) => (
              <motion.span
                key={i}
                custom={i}
                variants={charVariants}
                className={`inline-block ${char === " " ? "w-[0.3em]" : ""} ${i >= 7 ? "text-transparent bg-clip-text bg-gradient-to-r from-accent-warm via-white/90 to-accent-cool" : ""}`}
              >
                {char === " " ? "\u00A0" : char}
              </motion.span>
            ))}
        ...
        {/* Dual identity */}
        <div className="mt-8 md:mt-12 flex flex-col md:flex-row md:items-center gap-4 md:gap-0">
          <motion.div
            initial={{ opacity: 0, x: -40 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.6, ease, delay: 0.95 }}
          >
            <span className="font-display text-lg md:text-2xl lg:text-[2rem] font-semibold text-accent-warm glow-warm tracking-tight">
          ...
          >
            <span className="font-display text-lg md:text-2xl lg:text-[2rem] font-semibold text-accent-cool glow-cool tracking-tight">
              AI Engineer
            </span>
        ...
        <motion.div
          ...
          className="mt-8 md:mt-10 h-[1px] w-full max-w-md bg-gradient-to-r from-accent-warm via-white/20 to-accent-cool origin-left"
          aria-hidden="true"
        />
        ...
        <motion.p
          ...
          className="mt-6 md:mt-8 max-w-xl text-sm md:text-[15px] text-[var(--text-secondary)] leading-[1.8] font-body"
        >
        ...
        {/* Social links */}
        <motion.div className="mt-8 flex flex-wrap items-center gap-4 md:gap-6" initial="hidden" animate="visible">
          {[
            { href: "https://github.com/nikhil1057", label: "GitHub", icon: <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0024 12c0-6.63-5.37-12-12-12z"/></svg> },
            { href: "https://linkedin.com/in/nikhiltiwari1057", label: "LinkedIn", icon: <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg> },
            { href: "mailto:nikhiltiwari1057@gmail.com", label: "Email", icon: <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="M22 7l-10 6L2 7"/></svg> },
          ].map((link, i) => (
            <motion.a
              key={link.href}
              custom={i}
              ...
              className="group flex items-center gap-2 text-xs text-[var(--text-secondary)] hover:text-accent-cool transition-colors font-body focus:outline-none focus:ring-2 focus:ring-accent-cool/50 rounded px-1"
            >
              {link.icon}
